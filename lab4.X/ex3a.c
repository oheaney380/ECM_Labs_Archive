...
void main (void) {
    ...
    T0CONbits.T0CS=1; // use pin RC3 (button))
    T0CONbits.PSA=1; // disable pre scaler
    
    LCD_Init(); //Initialise the LCD
    SetLine(1); //Set Line 1
    
    LCD_String("Press the button:"); //Initial Display
    
    SetLine(2);
    LCD_String("Stroke the cat!");
    
    SetLine(1);
    
    while(1) {
        if (TMR0L != 0) {
            storeval++; //Funky debouncing - doesn't work?
            TMR0L = 0;
            sprintf(buf,"   Stroked %d times", storeval); //Why do we need three spaces?
