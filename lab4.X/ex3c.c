...
void main (void) {
    ...
    while(1) {        
        ...
        //this scales the ADC result so it is a maximum
        //of 5 and the result is rounded down to the nearest integer
        int_part=(unsigned int)(LDRresult/51); // i.e. 255 / 5V = 51
        //calculate the fractional part
        // ADC_result/0.51 returns a value out of 500, i.e. it keeps two
        // more sig figs than int_part has, we then subtract int_part*100
        // to get only the fractional part
        frac_part=(unsigned int)( LDRresult/0.51) - int_part*100;
        //display the result
        // remember to include leading zeros in the
        // fractional part - (%02) means 2 figures with leading zeros
        sprintf(buf,"   Happy Level:%d.%02d",int_part,frac_part);
