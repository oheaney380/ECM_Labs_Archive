...
void goForward(struct DC_motor *m_L, struct DC_motor *m_R)
{
    m_L->direction=1;
    m_R->direction=1;
    fullSpeed(m_L, m_R);
...
void goBackward(struct DC_motor *m_L, struct DC_motor *m_R)
{
    m_L->direction=0;
    m_R->direction=0;
    fullSpeed(m_L, m_R);
...
// sets Pins for ADC & LDR and also initialise the bits
void setPinsADC () {
    TRISAbits.RA3=1;
    ANSEL0bits.ANS3=1;
    /* Initialise ADC */
